...
import { Ship, Lock, Mail, ArrowRight, AlertCircle, Building, User, Briefcase, Eye, EyeOff, Car, Home } from 'lucide-react';
...
export function SignUpPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        fullName: '',
        companyName: '',
        role: isCaptCar ? 'car_renter' : isCaptHouse ? 'house_renter' : 'company' // default role
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Handle special 'admin' convenience
            const signUpEmail = formData.email.trim().toLowerCase() === 'admin' ? 'admin@admin.com' : formData.email;
            const signUpPassword = formData.password === 'admin' ? 'adminadmin' : formData.password;
            ...
            if (user) {
                ...
                const { error: profileError } = await supabase
                    ...
                    .upsert([
                        {
                            id: user.id,
                            email: signUpEmail, // Use mapped email
            ...
            <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="mb-8 text-center">
                    ...
                    <h1 className="text-2xl font-bold text-slate-900">
                        {isCaptCar ? 'Create Car Rental Account' : isCaptHouse ? 'Join CAPT HOUSE' : 'Create your account'}
                    </h1>
                    <p className="mt-2 text-slate-600">
                        {isCaptCar ? 'Join CAPT CAR for Premium Mobility' : isCaptHouse ? 'Find your perfect sanctuary' : 'Join CAPTB Commercial Suite'}
                    </p>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <select
                                    className="block w-full rounded-lg border border-slate-300 py-2.5 pl-10 pr-3 text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:bg-slate-100 disabled:text-slate-500"
                                    value={formData.role}
                                ...
                                >
                                    ...
                                    <option value="car_renter">Car Renter (Driver)</option>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className={`mt-2 group flex w-full items-center justify-center gap-2 rounded-lg px-4 py-2.5 font-semibold text-white transition-all disabled:opacity-70 ${isCaptCar
